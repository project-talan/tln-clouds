...
module "ses" {
  ...
  name = "${module.shared.prefix_group}-ses"
  custom_from_subdomain = ["no-reply"]
...
resource "aws_route53_record" "amazonses_verification_record" {
  ...
  type    = "TXT"
  ttl     = "1800"
  records = ["v=DMARC1;p=quarantine;rua=mailto:no-reply@${var.domain_name}"]
}

resource "aws_ses_template" "MyTemplate" {
  name    = "${module.shared.prefix_group}-welcome"
