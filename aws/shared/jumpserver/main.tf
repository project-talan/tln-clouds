...
resource "aws_instance" "jumpserver" {
  ...
  user_data_base64 = base64encode(templatefile("${path.module}/templates/template.sh.tftpl", {
    ...
    server_private_key = one(wireguard_asymmetric_key.server[*].private_key) != null ? one(wireguard_asymmetric_key.server[*].private_key) : ""
    client_public_key  = one(wireguard_asymmetric_key.client[*].public_key) != null ? one(wireguard_asymmetric_key.client[*].public_key) : ""
  }))
